...
- captions
- member properties
- miembros especiales como `Base`, `Ano Base`, `S1`, `T1`, etc.
- medidas
...
|---|---|---|---|---|---|
| `Consulta1` | principal | `pAno` | producto, fecha, precio | matrix principal | contiene reglas de negocio |
| `pAno` | parametro | no | anos | dropdown | excluye `Ano Base` |
| `Descripcion reporte` | auxiliar | no | descripcion | bloque de ayuda | texto fijo |
...
- El parametro `pAno` excluia `Ano Base`.
- La consulta principal dependia de propiedades como `Mes Numero`, `Numero`, `Cantidad dias` y `Numero1`.
- El reporte tenia reglas de negocio embebidas que habia que respetar:
  - ocultar el mes actual
  - renombrar productos
  - forzar `0` para ciertos meses del ano 2021
...
- tabs internas que en Cognos cambian de pagina o vista: en Report Builder conviene resolverlas con bookmarks y secciones/paginas internas, no con `Drillthrough` al mismo `.rdl`
